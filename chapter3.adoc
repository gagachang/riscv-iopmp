...
For MD 0 {le} _m_ {le} 30, `MDLCK.md[_m_]` locks `SRCMD_EN(_s_).md[_m_]` for all existing RRID _s_.
For MD 31 {le} _m_ {le} 62, `MDLCKH.mdh[_m_]` locks `SRCMD_ENH(_s_).mdh[_m_]` for all existing RRID _s_.
...
Register `MDCFGLCK` is designed to partially or fully lock the MDCFG table. `MDCFGLCK` consists of two fields: `MDCFGLCK.l` and `MDCFGLCK.f`. `MDCFG(_m_)` is locked if _m_< `MDCFGLCK.f`. `MDCFGLCK.f` is incremental-only. Any smaller value can not be written into it. The `MDCFGLCK.l` lock bit is used to lock `MDCFGLCK`.
