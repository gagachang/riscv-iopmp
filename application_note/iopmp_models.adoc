...
The Rapid-k Model simplifies MDCFG table lookup by implementing a fixed allocation scheme where each memory domain is assigned exactly _k_ entry indices. This eliminates the need for MDCFG table lookups, reducing access latency and hardware complexity while maintaining full SRCMD table flexibility.

*Characteristics*:

* Full SRCMD table with arbitrary RRID-to-MD associations
* Simplified MDCFG with constant _k_ entry indices per MD
...
.Block diagram of the Rapid-4 model implementation. The MDCFG table is simplified to a constant mapping where each memory domain is assigned exactly four entry indices.
...
The Compact-k Model maximizes area efficiency by combining both SRCMD and MDCFG simplifications. It implements direct RRID-to-MD mapping with fixed _k_ entry indices per memory domain, eliminating both table lookup mechanisms. This represents the most area-optimized IOPMP configuration.

*Characteristics*:

* Direct RRID-to-MD mapping (RRID _i_ → MD _i_)
* Fixed _k_ entry indices per MD with constant allocation
