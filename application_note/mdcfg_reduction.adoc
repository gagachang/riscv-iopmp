...
==== Each Memory Domain Has Exactly _k_ Entry Indices
...
This is achieved by defining that each memory domain is assigned exactly _k_ entry indices, where _k_ is a fixed value. By making _k_ a constant, the range of entry indexes for a given MD _m_ is no longer stored in a table but is *directly calculable* (e.g., Entry Indexes = `[_m_ * _k_, (_m_ * _k_) + _k_ - 1]`).
...
For a particular case of _k_=1 (`HWCFG3.md_entry_num`=0), every memory domain has exactly one entry index. Software can skip the concept of the memory domain when using such an IOPMP.
