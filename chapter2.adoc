...
For a legal RRID, the IOPMP determines the applicable entries in the following order:

1. The IOPMP looks up the SRCMD table using the RRID to identify the memory domains associated with the requester.
2. The IOPMP looks up the MDCFG table using the associated memory domains to identify the entries belonging to those memory domains.
