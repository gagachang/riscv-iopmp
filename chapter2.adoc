...
It's important to note that, generally speaking, a single RRID can be associated with multiple memory domains (MDs), and a MD can be associated with multiple RRIDs. The SRCMD table is one of the fundamental structures in IOPMP to indicate associations between RRIDs and MDs, which will be introduced in <<#SRCMD_TABLE, SRCMD Table>>.

The MDCFG table is one of the fundamental structures in IOPMP to define which entries belong to a MD, which will be introduced in <<#MDCFG_TABLE, MDCFG Table>>.
...
The SRCMD table is an array indexed by RRID that associates each requester with its memory domains. The table uses a bitmap approach where each RRID can be associated with multiple memory domains. When an IOPMP receives a transaction with RRID _s_, the IOPMP first looks up the SRCMD table to find out all the memory domains associated with that RRID. An IOPMP instance can support up to 65,535 RRIDs; the actual number of RRIDs can be implementation-defined and is indicated in `HWCFG1.rrid_num`. The SRCMD table defines the mapping from an RRID to its associated MDs.
...
The MDCFG table is used to map a memory domain to its own IOPMP entries. It can be viewed as a partition of the IOPMP entry array. The MDCFG table defines which memory domain every IOPMP entry belongs to. An IOPMP entry can belong to at most one memory domain, but a memory domain can own zero or more IOPMP entries. After retrieving all related IOPMP entries, an IOPMP checks the transaction according to them.
...
Programmers should maintain proper settings in the MDCFG table when `HWCFG0.enable` is set, where `MDCFG(_m_ + 1).t` should be greater than or equal to `MDCFG(_m_).t` for any legal MD m. Otherwise, the MDCFG table has an improper setting, that is, there exist two legal MDs _m_ and _k_, `MDCFG(_k_).t` > `MDCFG(_m_).t` for _k_ < _m_. In this case, the specification denotes the MD _m_ as having an improper setting.

When the MDCFG table has an improper setting, the resulting association of IOPMP entries to MDs is implementation-defined, but the following conditions must be held:
...
Examples of implementation-defined behaviors for handling improper MDCFG settings are provided in <<#IMPROPER_MDCFG_REFERENCE,Reference Behaviors of Improper Settings on MDCFG Table>>.

[NOTE]
====
For portability, programmers should ensure the MDCFG table has a proper setting during the runtime. In the case that the process of programming or initializing the MDCFG table causes an improper setting to occur transiently, software can deassociate affected MDs in the SRCMD table before programming the MDCFG table to avoid security issues. Even though the procedure may introduce extra steps, the table is not subject to change at such a high frequency as to hurt overall performance. Thus, the specification requires programmers to maintain proper settings instead of requiring specific hardware behaviors on improper settings.
====

[#IOPMP_priority_and_matching_logic]
=== Priority and Matching Logic
The IOPMP checks each transaction against its IOPMP entries to determine whether the transaction is allowed or denied.

The error type identifies the cause of an illegal transaction and is recorded in `ERR_INFO.etype` when the error capture record is implemented. Error reactions are described in <<#ERROR_REACTIONS,Error Reactions>>, and the error-type encodings are summarized in <<#ERROR_TYPES,Summary of error types>>.
...
For a legal RRID, the IOPMP determines the applicable IOPMP entries in the following order:

1. The IOPMP looks up the SRCMD table using the RRID to identify the memory domains associated with the requester.
2. The IOPMP looks up the MDCFG table using the associated memory domains to identify the IOPMP entries belonging to those memory domains.
3. The IOPMP applies the entry priority and matching rules below to those entries.
...
* Return an implementation-defined bus error or a successful response with an implementation-defined value from receiver ports.
...
An error capture only occurs when there is no pending error, that is, `ERR_INFO.v` = ‘0’. If a pending error exists (`v` = ‘1’), the record will not be updated, even if a new illegal access is detected. In other words, `v` indicates whether the content of the capture record is valid and should be intentionally cleared in order to capture subsequent illegal accesses. Software acknowledges the captured violation by writing 1 to `ERR_INFO.v`, which clears the bit and reactivates the recorder.
The error capture record ((`ERR_INFO.ttype`, `ERR_INFO.etype`, `ERR_REQADDR`, `ERR_REQADDRH`, `ERR_REQID` and `ERR_USER(0)` ~ `ERR_USER(7)`)) is optional.
If it is not implemented, `HWCFG0.no_err_rec` must be wired to one.
It is possible to implement the error capture record without the error entry index record (`ERR_REQID.eid`).
In this case, `eid` must be wired to 0xffff.
