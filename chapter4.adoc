...
- If `MDCFG(_m_-1).t` {le} _j_ < `MDCFG(_m_).t`, where m > 0.
- If _j_ < `MDCFG(0).t` where m = 0.
- If `MDCFG(_m_-1).t` > `MDCFG(_m_).t`, then the MDCFG table is improperly programmed. Please refer <<#MDCFG_TABLE, MDCFG Table>> for IOPMP behavior of improperly programming.
