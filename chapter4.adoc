[[Registers]]
== Registers
If an optional register is not implemented, the behavior is implementation-defined unless otherwise specified. An optional field in an implemented register means being WARL. If it is not programmable, it should be hardwired to value matching its meaning and should not cause any effect when written. Reserved regions are reserved for standard use. The behavior in the reserved regions is implementation-defined.
...
|{set:cellbgcolor:#FFFFFF}
 0x0040 | `MDLCK`    | Lock register for lower MDs in the SRCMD table.
|0x0044 | `MDLCKH`   | (Optional) lock register for higher MDs in the SRCMD table.
|0x0048 | `MDCFGLCK` | Lock register for the MDCFG table.
...
|{set:cellbgcolor:#FFFFFF}
0x0800 + (_m_) × 4 | `MDCFG(_m_)`  |MD config register, which specifies the indices of IOPMP entries belonging to an MD.
...
|`no_err_rec`   |23:23  |R      |IMP       |Indicate if error record (`ERR_INFO.ttype`, `ERR_INFO.etype`, `ERR_REQADDR`, `ERR_REQADDRH`, `ERR_REQID` and `ERR_USER(0)` ~ `ERR_USER(7)`) is not implemented.
...
|`tor_en`       |31:31  |R      |IMP       |Indicate if TOR address-matching mode is supported. See <<#ADDRESS_MATCHING_MODES, Address Matching Modes>>.
|===
...
|`entry_num` |31:16  |R      |IMP        |Indicate the number of IOPMP entries implemented in the instance. The legal range is 1 to 65535; the value zero is reserved.
|===
...
|===
|Field      |Bits       |R/W    |Default    |Description
|`l`        |0:0        |W1SS   |IMP        | `MDLCK.l` lock bit for the `MDLCK` and `MDLCKH` registers.
...
`MDCFGLCK` is the lock register for the MDCFG table.

[cols="<2,<1,<1,<1,<6",options="header"]
|===
|Field     |Bits       |R/W    |Default    |Description
|`l`       |0:0        |W1SS   |IMP        | `MDCFGLCK.l` lock bit for the `MDCFGLCK` register.
...
If `f` {ge} `HWCFG0.md_num`, all MDCFG entries are locked.
...
|===
|Field   |Bits       |R/W    |Default    |Description
|`l`     |0:0        |W1SS   |IMP        | `ENTRYLCK.l` lock bit for the `ENTRYLCK` register.
...
If `f` {ge} `HWCFG1.entry_num`, all IOPMP entries are locked.
|`rsv`   |31:17      |ZERO   |0          | Must be zero on write, reserved for future
|===

=== Error Capture Registers

Error status bits that are cleared by software use the RW1C convention unless otherwise specified.
RW1C allows software to acknowledge selected pending status bits by writing 1, while writing 0 leaves the status unchanged.
This convention helps avoid accidentally clearing adjacent status bits during read-modify-write sequences and prevents software from writing arbitrary values into hardware-generated status fields.
...
|===
|Field  |Bits   |R/W    |Default    |Description
|`l`    |0:0    |W1SS   |IMP        |`ERR_CFG.l` lock bit for the `ERR_CFG` register.
|`ie`   |1:1    |WARL   |IMP        |Enable the interrupt of the IOPMP rule violation.
|`rs`   |2:2    |WARL   |IMP       a|

To suppress an error response on an IOPMP rule violation.

* 0x0: respond an implementation-defined error, such as a bus error
* 0x1: respond a success with an implementation-defined value to the requester instead of an error
...
|===
|Field     |Bits   |R/W   |Default |Description
|`v`       |0:0    |RW1C  |0       | Indicates whether the illegal capture recorder (`ERR_REQID`, `ERR_REQADDR`, `ERR_REQADDRH`, `ERR_INFO.ttype`, and `ERR_INFO.etype`) contains valid information. When a violation is captured, `v` is set and remains set until software acknowledges the captured violation. The associated captured error fields are hardware-generated and read-only, so software cannot forge the captured fault context through these registers. An interrupt will be triggered if a violation is detected and related interrupt enable/suppression configure bits are not disabled, and the interrupt will remain asserted until `v` is cleared. +
Writing 1 to `ERR_INFO.v` acknowledges the captured violation, clears `v`, reactivates the illegal recorder, and deasserts the interrupt if no other interrupt condition remains pending. Writing 0 causes no effect on the bit.
...
|`etype`   |7:4    |R     |0      a| Indicates the type of violation, also referred to as the error type.
...
The MDCFG table is a lookup to specify the number of IOPMP entries that is associated with each MD.
...
|===
|Field  |Bits       |R/W    |Default    |Description
|`t`    |15:0       |WARL   |DC/IMP    a| Indicate the upper bound of the IOPMP entry index range of memory domain _m_. An IOPMP entry with index _j_ belongs to MD _m_

- if `MDCFG(_m_-1).t` {le} _j_ < `MDCFG(_m_).t`, where _m_ > 0; or
- if _j_ < `MDCFG(0).t`, where _m_ = 0.

If `MDCFG(_m_-1).t` {ge} `MDCFG(_m_).t`, no IOPMP entry belongs to MD _m_. Please refer to <<#MDCFG_TABLE, MDCFG Table>> for improper settings.
...
This section uses _i_ to denote the index of an IOPMP entry. Each register below that is implemented has instances for _i_ = 0...`HWCFG1.entry_num`-1; no register instance is implemented for any greater _i_.
...
|`a`     |4:3        |WARL   |DC        a|The address-matching mode of the IOPMP entry. See <<#ADDRESS_MATCHING_MODES, Address Matching Modes>>.
