...
[#EXT_SUMMARY_SRCMD]
.Register summary of extensions in the SRCMD table.
[%autowidth, options="header"]
|===
...
|`stall_violation_en` |4:4    |WARL   |IMP        |Indicates whether the IOPMP faults stalled transactions. When the bit is set, the IOPMP faults the transactions if the corresponding RRID is not exempt from stall and the IOPMP cannot accommodate more stall transactions.
...
`ERR_MSIADDR` indicates the lower MSI message address for error reporting.
[cols="<2,<1,<1,<1,<6",options="header"]
|===
|Field         |Bits       |R/W    |Default    |Description
...
`ERR_MSIADDRH` indicates the higher MSI message address for error reporting.
[cols="<2,<1,<1,<1,<6",options="header"]
|===
|Field         |Bits       |R/W    |Default    |Description
...
* 0x1: do not respond an error. User to define the behavior, e.g., respond a success with an implementation-defined value to the requester.
