...
The per-entry suppression extension only extends the <<#ERROR_REPORTING,error reporting>> of illegal transactions in which an entry is matched but does not grant transaction permission to operate.
