...
.An example block diagram of an IOPMP with the SPS extension. It illustrates the checking flow of an IOPMP with the SPS extension. The IOPMP first looks up the SRCMD table according to the RRID carried by the incoming transaction to retrieve associated MD indices and the corresponding permissions related to these MDs. By the MD indices, the IOPMP looks up the MDCFG table to get the belonging entry indices. The final step checks the access right according to the above entry indices and corresponding permissions.
